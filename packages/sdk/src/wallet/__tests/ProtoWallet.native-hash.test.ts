...
describe('ProtoWallet large signature payload hashing', () => {
  ...
  const parameters = {
    protocolID: [0, 'native hash parity'] as [0, string],
    keyID: 'test',
    counterparty: 'self'
  }
  ...
  it('preserves deterministic signatures and verifies actual native SHA-256 results', async () => {
    ...
    install(digest)
    const expected = await wallet.createSignature({
      ...parameters,
      hashToDirectlySign: Hash.sha256(data)
    })
    const actual = await wallet.createSignature({ ...parameters, data })
    expect(actual).toEqual(expected)
    await expect(wallet.verifySignature({ ...parameters, data, ...actual })).resolves.toEqual({
      valid: true
    })
    ...
    changed[0] ^= 1
    await expect(
      wallet.verifySignature({ ...parameters, data: changed, ...actual })
    ).rejects.toMatchObject({ code: 'ERR_INVALID_SIGNATURE' })
  })
  ...
  it('keeps short payloads and explicit digests on their existing path', async () => {
    ...
    await expect(
      wallet.verifySignature({ ...parameters, data: short, ...signature })
    ).resolves.toEqual({ valid: true })
    const hash = Hash.sha256(data)
    const direct = await wallet.createSignature({ ...parameters, hashToDirectlySign: hash })
    await expect(
      wallet.verifySignature({ ...parameters, hashToDirectlyVerify: hash, ...direct })
    ).resolves.toEqual({ valid: true })
    await expect(
      wallet.createSignature({ ...parameters, data, hashToDirectlySign: hash })
    ).rejects.toThrow('exactly one value')
    expect(digest).not.toHaveBeenCalled()
  })

  it('rejects non-byte values instead of coercing the signed payload', async () => {
    const digest = jest.fn((algorithm, bytes) => webcrypto.subtle.digest(algorithm, bytes))
    install(digest)
    const unusual = [...data]
    unusual.splice(0, 4, -1, 256, 1.75, Number.NaN)
    await expect(wallet.createSignature({ ...parameters, data: unusual })).rejects.toThrow(
      'dense array of bytes'
    )
    expect(digest).not.toHaveBeenCalled()
  })
  ...
  it('works in hosts without Web Crypto', async () => {
    ...
    await expect(wallet.verifySignature({ ...parameters, data, ...signature })).resolves.toEqual({
      valid: true
    })
  })

  it.each(['reject', 'malformed'] as const)(
    'falls back to the original byte snapshot after a %s native result',
    async failure => {
      let finish: () => void = () => {
        throw new Error('digest did not start')
      }
      const digest = jest.fn(
        () =>
          new Promise<ArrayBuffer>((resolve, reject) => {
            finish = () =>
              failure === 'reject'
                ? reject(new Error('unsupported host'))
                : resolve(new ArrayBuffer(31))
          })
      )
      install(digest)
      const expected = await wallet.createSignature({
