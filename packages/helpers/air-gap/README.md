...
The decoder is the untrusted surface of this package, and its resources are bounded regardless of what a camera — or a hostile sender — feeds it: scanned strings longer than any legal part are rejected before base64 decoding, solved-block state grows only with actual progress, duplicate tracking is capped (`65,536` sequence numbers), and buffered mixes are budgeted (`1,024` parts / `4,096` unresolved block references). The deterministic Fisher–Yates mapping uses sparse swaps, so neither starting a maximum-`K` session nor mapping an ordinary part allocates an array proportional to `K`; a fountain part whose derived degree alone exceeds the index budget is rejected before its index set is built. Systematic and degree-1 parts are never buffered, so a looping honest sender always completes even against a full buffer. The CRC is an integrity check only: an adversary who can show codes to your camera can forge any header, so **authenticate the payload inside the payload** (signature or MAC) whenever it matters.
