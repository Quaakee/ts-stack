...
function optionalHeaderText(value: unknown, label: string, maximumBytes: number): string | undefined {
...
function validateHttpClient(value: unknown): HttpClient {
  ...
  if (client == null || typeof client !== 'object' || typeof (client as HttpClient).request !== 'function') {
...
export function normalizeArcProviderConfig(
  config: string | ArcConfig | undefined,
  defaultDeploymentId: () => string
): NormalizedArcProviderConfig {
  ...
  return Object.freeze({
    ...
    deploymentId: optionalHeaderText(read('deploymentId'), 'ARC deployment ID', 256) ?? defaultDeploymentId(),
