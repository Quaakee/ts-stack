...
interface ExactResumeCandidate {
  txid: string
...
/**
 * Reclaim failed actions only when their persisted signed lineage is unambiguous.
 *
 * Every candidate is validated read-only first. `isCompleteWith` then decides, before any
 * state is committed, whether the surrounding sendWith set can be sent together (#59 F1).
 * An incomplete set commits no retry, so nothing is later broadcast on its own.
 */
export async function resumeFailedSendWith(
  storage: StorageProvider,
  userId: number,
  txids: string[],
  isCompleteWith: (candidates: ReadonlySet<string>) => Promise<boolean>
): Promise<void> {
  ...
  if (!(await isCompleteWith(new Set(candidates.map(candidate => candidate.txid))))) return
  for (const candidate of candidates) await commitExactResume(storage, userId, candidate)
}
...
async function commitExactResume(
  ...
  await storage.transaction(async trx => {
    ...
    if (!resumableStatuses.includes(req.status)) return
    if (
