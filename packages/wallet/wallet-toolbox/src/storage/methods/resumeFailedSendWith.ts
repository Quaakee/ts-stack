...
/**
 * Plan the exact retries in a sendWith set, read-only: nothing is committed here (#59).
 * Each failed request's persisted signed lineage must be unambiguous and verify.
 */
...
/**
 * Admit planned retries into the set's one share lookup `r` (#59 R2-1).
 *
 * A candidate is admitted only while `r` still shows its planned failed request, and only when
 * every other member of `r` is already sendable. Admitted details become `readyToSend` (an
 * immediate share also merges their planned signed bytes into `r.beef`), so the share's
 * completeness gate reads this same decision. Otherwise `r` keeps the failed details, the gate
 * refuses the whole set and no retry is ever committed.
 */
export async function admitExactResumes(
  storage: StorageProvider,
  ...
  if (!isDelayed) {
    for (const candidate of admitted) {
      try {
        // As the lookup merges each ready request; these are the bytes the commit re-checks.
        await storage.mergeReqToBeefToShareExternally(candidate.original, r.beef, [])
      } catch (error_: unknown) {
        const e = WalletError.fromUnknown(error_)
        detailOf(candidate.txid)!.error = `${e.name}: ${e.message}`
        return []
      }
    }
  }
  for (const candidate of admitted) detailOf(candidate.txid)!.status = 'readyToSend'
  return admitted
}

/**
 * Commit every admitted retry inside the caller's one storage transaction, all or nothing (#59 R2-1).
 *
...
async function planExactResume(
  ...
  return { txid, original, raw, preparedAction, lockedOutputIds }
}
