...
| `relayUrl`                     | `string`                          | No       | `process.env.RELAY_URL` → `ws://localhost:3000` | WebSocket base URL of this server. Production addresses must use `wss://`; `ws://` is accepted only on loopback. Returned by `GET /api/session/:id` so the mobile can resolve it after scanning the QR. Not embedded in the QR itself.                                                                                                                                                                                                                                                                                |
| `origin`                       | `string`                          | No       | `process.env.ORIGIN` → `http://localhost:5173`  | Default canonical origin embedded in the QR when `createSession()` has no per-session override. Production and physical-device origins require HTTPS; HTTP is accepted only on loopback. The mobile resolves the relay through `{origin}/api/session/{topic}`, so the origin's TLS certificate is the trust anchor. Multi-app deployments may pass `origin` per call and constrain it with `allowedOrigins`; split-device development should use a trusted HTTPS tunnel.                                              |
...
| `maxSessions`                  | `number`                          | No       | `1000`                                          | Maximum number of sessions and relay topics held in memory at once (1–100000). `GET /api/session` returns HTTP 429 when the limit is reached.                                                                                                                                                                                                                                                                                                                                                                         |
| `maxSessionCreationsPerMinute` | `number`                          | No       | `120`                                           | Maximum new pairing sessions created by one service instance per minute (1–100000). Excess requests receive HTTP 429. Add a source-aware deployment-edge limit for distributed abuse control.                                                                                                                                                                                                                                                                                                                         |
...
| `mobileAuthTimeoutMs`          | `number`                          | No       | `15000`                                         | Deadline (1–120000 ms) for every newly opened or reconnecting mobile socket to prove identity-key possession with a valid encrypted `pairing_approved` message.                                                                                                                                                                                                                                                                                                                                                       |     |
...
The QR is a short-lived bearer invitation: its signature authenticates the backend, not an expected phone. Anyone who obtains it before expiry can pair their own wallet first. Do not log, capture, analyze, or screen-share `qrDataUrl` or `pairingUri`; keep privileged methods behind `onApprovalRequired`. A mandatory pre-identified-phone confirmation requires a versioned handshake and is not part of the current compatible wire protocol.

The `desktopToken` is a cryptographically random bearer secret. It must be sent as `X-Desktop-Token` on every HTTP wallet request. For a direct desktop-role browser WebSocket, offer `['bsv-wallet-relay', 'bsv-wallet-relay-token.' + desktopToken]` as subprotocols; the server echoes only the stable first value. The legacy `?token=` form remains accepted for compatibility but new clients must not put the bearer in routinely logged URLs. Possession grants the ability to request wallet operations: never log it, embed it in the QR, or share it with the mobile. `WalletRelayClient` keeps it in `sessionStorage` by default for same-tab refresh; set `persistSession: false` when that persistence is inappropriate.
...
Encrypts an RPC call, sends it to the paired mobile wallet over WebSocket, and waits for the response. Rejects after 30 seconds if no response arrives. Throws if the session is not connected or if `desktopToken` does not match. Responses must match the request's session, RPC ID, and sequence; known BRC-100 wallet result shapes and security verdicts are validated before resolution.
...
Both session routes emit `Cache-Control: no-store` and `Pragma: no-cache`. Preserve those headers through reverse proxies and CDNs: the creation response contains the wallet-equivalent `desktopToken`. Custom framework adapters must apply the same response policy and a strict bounded request-body parser (the bundled Express scaffold uses 64 KiB).
...
| `persistSession`        | `boolean`                          | `true`                            | Persist the active session and bearer `desktopToken` to `sessionStorage` so a same-tab refresh can resume it. Disable on higher-assurance pages or wherever same-origin script exposure is unacceptable. `disconnect()` clears the stored secret. |
...
| `maxLogEntries`         | `number`                           | `100`                             | Maximum in-memory request-log entries, from 0 to 10,000. Set 0 to disable retention. Results can contain sensitive wallet data; expose the log only in an appropriately trusted UI.                                                               |
...
Makes a `GET` request to `{params.origin}/api/session/{params.topic}` over HTTPS and reads the `relay` field from the response. The origin's TLS certificate is the trust anchor — the relay itself may be on any domain. Redirects are not followed; the request has a 10-second deadline and a 256 KiB response limit. The returned session identifier must match the requested topic, and production relay URLs must use `wss://`.
...
- Connected session TTL: 24 hours (sessions are GC'd from memory after this)
...
| Option        | Type     | Default | Description                                                                                                          |
| ------------- | -------- | ------- | -------------------------------------------------------------------------------------------------------------------- |
| `maxSessions` | `number` | `1000`  | Maximum concurrent sessions (1–100000). `createSession()` throws with a `429`-style error when the limit is reached. |
...
interface Session {
  ...
  expiresAt: number // Unix ms (short pending lifetime; extended to 24 h when connected)
...
interface SessionInfo {
  ...
  desktopToken?: string // present on creation — use X-Desktop-Token or the documented WS subprotocol
}
```
