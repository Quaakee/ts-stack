...
const identity = async (w: ProtoWallet): Promise<string> =>
  (await w.getPublicKey({ identityKey: true })).publicKey
...
async function build(
  opts: {
    ...
    sanctioned?: string[]
    screeningVerdict?: unknown
    extraP2pkhInput?: boolean
  } = {}
): Promise<Built> {
  const [payerKey, receiverKey, verifierKey] = await Promise.all([
    identity(payer),
    identity(receiver),
    identity(overlay)
  ])
  const { publicKey: ownChildKey } = await spender.getPublicKey({
    protocolID,
    keyID,
    counterparty: payerKey,
    forSelf: true
  })
  const { publicKey: receiverChildKey } = await spender.getPublicKey({
    protocolID,
    keyID,
  ...
  source.addOutput({
    lockingScript: new MandalaToken().lock(assetId, 100, pkhOf(opts.coinKey ?? ownChildKey)),
    satoshis: 1
  })
  ...
  tx.addOutput({
    lockingScript: new MandalaToken().lock(assetId, 100, pkhOf(receiverChildKey)),
    satoshis: 1
  })

  const inputLinkage = await spender.revealSpecificKeyLinkage({
    counterparty: payerKey,
    verifier: verifierKey,
    protocolID,
    keyID
  })
  const outputLinkage = await spender.revealSpecificKeyLinkage({
    counterparty: receiverKey,
    verifier: verifierKey,
    protocolID,
    keyID
  ...
  const rows = opts.rows ?? {
    [`${sourceId}.0`]: {
      ...
      assetId,
      amount: 100,
      identityKey: await identity(spender),
      createdAt: new Date()
    }
  }
  const tm = new MandalaTopicManager({
    ...
    screeningProvider:
      opts.screeningVerdict === undefined
        ? new InMemoryScreeningProvider(opts.sanctioned ?? [])
        : { isSanctioned: async () => opts.screeningVerdict as boolean },
  ...
  return {
    ...
    inputLinkage,
    outputLinkage,
    previousCoins: opts.extraP2pkhInput === true ? [0, 1] : [0]
  }
}
...
describe('MandalaTopicManager spend identity', () => {
  it('screens the stored spender after optional linkage verification', async () => {
    ...
    expect(
      (
        await clean.tm.identifyAdmissibleOutputs(
          clean.beef,
          clean.previousCoins,
          payloadWith(clean, true)
        )
      ).outputsToAdmit
    ).toEqual([0])
    ...
    await expect(
      flagged.tm.identifyAdmissibleOutputs(
        flagged.beef,
        flagged.previousCoins,
        payloadWith(flagged, true)
      )
    ).rejects.toThrow('sanctioned')
  })

  it('fails closed when a screening provider returns a non-boolean verdict', async () => {
    const b = await build({ screeningVerdict: null })
    await expect(
      b.tm.identifyAdmissibleOutputs(b.beef, b.previousCoins, payloadWith(b, true))
  ...
  it('accepts a linkage that agrees with the stored owner, case-insensitively', async () => {
    ...
    expect(
      (await b2.tm.identifyAdmissibleOutputs(b2.beef, b2.previousCoins, payloadWith(b2, true)))
        .outputsToAdmit
    ).toEqual([0])
  })
  ...
  it('without a linkage, screens the stored owner and rejects missing ownership', async () => {
    ...
    const owned = await build({
      rows: { [outpoint]: ownerRow(strangerKey, outpoint) },
      sanctioned: [strangerKey]
    })
    await expect(
      owned.tm.identifyAdmissibleOutputs(owned.beef, owned.previousCoins, payloadWith(owned, false))
    ).rejects.toThrow('sanctioned')

    const unknown = await build({ rows: {}, sanctioned: [strangerKey] })
    for (const withLinkage of [false, true]) {
      await expect(
        unknown.tm.identifyAdmissibleOutputs(
          unknown.beef,
          unknown.previousCoins,
          payloadWith(unknown, withLinkage)
        )
  ...
  it('ignores a previous coin that is not a token output', async () => {
    ...
    expect(
      (await b.tm.identifyAdmissibleOutputs(b.beef, b.previousCoins, payloadWith(b, true)))
        .outputsToAdmit
    ).toEqual([0])
  })
  it('rejects blank owner identities in stored rows', async () => {
    ...
    for (const identityKey of ['', '  ']) {
      ...
      await expect(
        b.tm.identifyAdmissibleOutputs(b.beef, b.previousCoins, payloadWith(b, false))
      ).rejects.toThrow('missing verified owner')
    }
  })
