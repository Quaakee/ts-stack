...
export class MandalaStorageManager {
  ...
  private readonly indexes = new CollectionIndexes('MandalaStorageManager', () => [
    ...
    {
      label: 'mandalaMetadata txid_1_outputIndex_1',
    ...
    {
      label: 'mandalaAssetStates assetId_1',
      collection: this.assetStates,
      keys: { assetId: 1 },
      options: { unique: true }
    },
    {
      label: 'mandalaAdminHistory assetId_1_height_1_offset_1_admitSeq_1',
      collection: this.adminHistory,
      keys: { assetId: 1, height: 1, offset: 1, admitSeq: 1 }
    }
  ])
  ...
  /**
   * Atomically stores a newly admitted token without replacing an existing
   * outpoint. Returns true only to the callback that created the row, so
   * idempotent replay cannot increment the cached balance twice.
   */
  async storeTokenIfAbsent(record: MandalaTokenRecord): Promise<boolean> {
    await this.ensureIndexes()
    ...
    if (
      ...
      existing.identityKey.toLowerCase() !== record.identityKey.toLowerCase()
    ) {
      throw new Error('Conflicting Mandala token record for an existing outpoint')
    }
    return false
  }
  ...
  /** Atomically removes and returns one token row for idempotent spend/eviction accounting. */
  async takeToken(txid: string, outputIndex: number): Promise<MandalaTokenRecord | null> {
    await this.ensureIndexes()
    return await this.tokens.findOneAndDelete({ txid, outputIndex })
  }

  async findByAssetId(assetId: string, limit?: number, skip = 0): Promise<UTXOReference[]> {
    await this.ensureIndexes()
    const state = await this.getAssetState(assetId)
    const evicted = new Set(state.evictedOutpoints.map(outpoint => outpoint.toLowerCase()))
    let cursor = this.tokens
      .find({ assetId })
      .project<UTXOReference>({ txid: 1, outputIndex: 1, _id: 0 })
      .skip(skip)
    if (limit !== undefined) cursor = cursor.limit(limit)
    const rows = await cursor.toArray()
    return rows.filter(r => !evicted.has(`${r.txid}.${r.outputIndex}`.toLowerCase()))
  }

  async findByOutpoint(txid: string, outputIndex: number): Promise<UTXOReference[]> {
    await this.ensureIndexes()
    return await this.tokens
      .find({ txid, outputIndex })
      .project<UTXOReference>({ txid: 1, outputIndex: 1, _id: 0 })
      .toArray()
  }
  ...
  async findMetadataByAssetId(assetId: string, limit?: number, skip = 0): Promise<UTXOReference[]> {
    await this.ensureIndexes()
    let cursor = this.metadata
      .find({ assetId })
      .project<UTXOReference>({ txid: 1, outputIndex: 1, _id: 0 })
      .skip(skip)
    if (limit !== undefined) cursor = cursor.limit(limit)
