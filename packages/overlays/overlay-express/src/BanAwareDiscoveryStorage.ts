import type { SHIPStorage, SLAPStorage } from '@bsv/overlay-discovery-services'
...
export class BanAwareSHIPStorage {
  ...
  async storeSHIPRecord(
    ...
    if (await this.banService.isOutpointBanned(txid, outputIndex)) {
      this.logger.log(
        `[BAN] Blocked banned outpoint from SHIP storage: txid=${serializeLogValue(txid)} outputIndex=${serializeLogValue(outputIndex)}`
      )
      return
    }
    if (await this.banService.isDomainBanned(domain)) {
      this.logger.log(
        `[BAN] Blocked banned domain from SHIP storage: domain=${serializeLogValue(domain)} txid=${serializeLogValue(txid)} outputIndex=${serializeLogValue(outputIndex)}`
      )
      return
    }
  ...
  async findRecord(
    ...args: Parameters<SHIPStorage['findRecord']>
  ): ReturnType<SHIPStorage['findRecord']> {
...
export class BanAwareSLAPStorage {
  ...
  async storeSLAPRecord(
    ...
    if (await this.banService.isOutpointBanned(txid, outputIndex)) {
      this.logger.log(
        `[BAN] Blocked banned outpoint from SLAP storage: txid=${serializeLogValue(txid)} outputIndex=${serializeLogValue(outputIndex)}`
      )
      return
    }
    if (await this.banService.isDomainBanned(domain)) {
      this.logger.log(
        `[BAN] Blocked banned domain from SLAP storage: domain=${serializeLogValue(domain)} txid=${serializeLogValue(txid)} outputIndex=${serializeLogValue(outputIndex)}`
      )
      return
    }
  ...
  async findRecord(
    ...args: Parameters<SLAPStorage['findRecord']>
  ): ReturnType<SLAPStorage['findRecord']> {
    return await this.wrapped.findRecord(...args)
  }
