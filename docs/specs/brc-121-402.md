...
policy are still stateful security dependencies. The built-in replay store is
bounded and process-local; multi-process services require shared durable atomic
claim state.
...
5. **Server** (validates & serves)
   ...
   - Requires an exact `accepted === true` result and rejects an exact `isMerge === true` detail
   - Atomically claims the payment transaction ID; only the first claim can serve the resource
   - If the transaction, nonce, amount, or output index is invalid → reject (HTTP 402)
   - If amount < required → reject (HTTP 402)
   - If wallet or replay-store state is ambiguous → return HTTP 503 without a fresh challenge
