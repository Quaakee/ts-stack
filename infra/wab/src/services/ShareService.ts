...
// Server encryption key - must be exactly 32 bytes (256 bits) for AES-256.
// In production, this should come from a secure key management system (HSM, KMS).
export function readShareEncryptionKey(value = process.env.SHARE_ENCRYPTION_KEY): Buffer {
  if (value == null || value.length === 0) {
    throw new Error('SHARE_ENCRYPTION_KEY environment variable not set')
  }
  if (!/^[0-9a-fA-F]{64}$/.test(value)) {
    throw new Error('SHARE_ENCRYPTION_KEY must be exactly 64 hexadecimal characters (32 bytes)')
  }
  return Buffer.from(value, 'hex')
}

export function validateShareEncryptionConfig(): void {
  readShareEncryptionKey()
}
...
export class ShareService {
  ...
  static decryptShare(encrypted: Buffer, nonce: string, tag: string): string {
    const key = this.getEncryptionKey()
    if (!/^[0-9a-fA-F]{24}$/.test(nonce)) {
      throw new Error('Encrypted share nonce must be exactly 12 bytes of hexadecimal data')
    }
    if (!/^[0-9a-fA-F]{32}$/.test(tag)) {
      throw new Error(
        'Encrypted share authentication tag must be exactly 16 bytes of hexadecimal data'
      )
    }
    const nonceBuffer = Buffer.from(nonce, 'hex')
    const tagBuffer = Buffer.from(tag, 'hex')

    const decipher = createDecipheriv('aes-256-gcm', key, nonceBuffer, { authTagLength: 16 })
  ...
  static async storeShare(userId: number, share: string): Promise<ShamirShareEntity> {
    ...
    const result = await db<ShamirShareEntity>('shamir_shares').where({ id }).first()
    ...
    if (attemptCount >= config.maxAttempts) {
      ...
      if (oldestAttempt) {
        const lockoutEnd = new Date(
          new Date(oldestAttempt.timestamp).getTime() + config.lockoutMinutes * 60 * 1000
        )
        const now = new Date()

        if (lockoutEnd > now) {
          const retryAfterMinutes = Math.ceil((lockoutEnd.getTime() - now.getTime()) / 60000)
   ...
   */
  static async getAccessLogs(userId: number, limit: number = 10): Promise<ShareAccessLogEntity[]> {
