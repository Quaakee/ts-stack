...
export class AuthController {
  ...
  public static async startAuth(req: Request, res: Response) {
    try {
      ...
      const { methodType, presentationKey, payload } = body
      if (
      ...
      log.error(
        { operation: 'controller.auth.start', err: error, outcome: 'error' },
        'startAuth failed'
      )
  ...
  public static async completeAuth(req: Request, res: Response) {
    try {
      ...
      log.error(
        { operation: 'controller.auth.complete', err: error, outcome: 'error' },
        'completeAuth failed'
      )
      res.status(500).json({ message: 'An internal error occurred.' })
    }
