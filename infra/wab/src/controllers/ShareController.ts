...
import { AuthIdentityConflictError, UserService } from '../services/UserService'
...
type VerifiedIdentity =
  { success: true; config: string; userId?: number } | { success: false; message: string }
...
async function verifyIdentity(
  ...
  return user ? { success: true, config, userId: user.id } : { success: true, config }
}
...
type UserResolution =
  { success: true; user: User } | { success: false; status: 401 | 403 | 404 | 429; message: string }
...
async function resolveStoreUser(
  ...
  if (identity.userId !== undefined) {
    const user = await UserService.attachUserIdHash(identity.userId, userIdHash)
...
function parseShareTargetRequest(body: unknown): ShareTargetRequest | undefined {
  ...
  if (!isAuthMethodType(methodType) || !isAuthPayload(payload) || !isHexIdentifier(userIdHash)) {
    return undefined
  }
...
async function authorizeShareUser(
  ...
  const rateLimit = await ShareService.isRateLimited(user.id, ipAddress, action)
  if (rateLimit.limited) {
    await ShareService.logAccess(user.id, ipAddress, action, false, 'Rate limited')
    return {
      success: false,
      status: 429,
  ...
  const identity = await verifyIdentity(request.methodType, request.userIdHash, request.payload)
  if (!identity.success) {
    await ShareService.logAccess(user.id, ipAddress, action, false, 'OTP verification failed')
    return { success: false, status: 401, message: identity.message }
  }
...
function sendResolutionFailure(res: Response, resolution: UserResolution & { success: false }) {
...
export class ShareController {
  ...
  public static async storeShare(req: Request, res: Response) {
    ...
    try {
      ...
      const { methodType, payload, shareB, userIdHash } = body
      ...
      const resolution = await resolveStoreUser(methodType, payload, userIdHash, ipAddress)
      ...
      return handleShareError(res, error, 'controller.share.store', 'storeShare failed', true)
    }
  }
  ...
  public static async retrieveShare(req: Request, res: Response) {
    ...
    try {
      const request = parseShareTargetRequest(snapshotRequestBody(req.body))
      if (!request) {
        return res.status(400).json({
          success: false,
      ...
      const resolution = await authorizeShareUser(request, ipAddress, 'retrieve')
      if (!resolution.success) {
        return sendResolutionFailure(res, resolution)
      }
      const { user } = resolution
      ...
      return handleShareError(res, error, 'controller.share.retrieve', 'retrieveShare failed')
    }
  }
  ...
  public static async updateShare(req: Request, res: Response) {
    ...
    try {
      const body = snapshotRequestBody(req.body)
      const request = parseShareTargetRequest(body)
      const newShareB = body?.newShareB
      ...
      const resolution = await authorizeShareUser(request, ipAddress, 'update')
      if (!resolution.success) {
        return sendResolutionFailure(res, resolution)
      }
      const { user } = resolution
      ...
      return handleShareError(res, error, 'controller.share.update', 'updateShare failed')
    }
  }
  ...
  public static async deleteUser(req: Request, res: Response) {
    ...
    try {
      ...
      // Delete the account while detaching durable faucet-abuse evidence.
      await UserService.deleteUserByUserIdHash(request.userIdHash)

      res.json({
        success: true,
        message:
          'Account and share deleted. Detached authentication identity and faucet-payment evidence are retained only to enforce the one-time faucet policy.'
      })
    } catch (error: unknown) {
      return handleShareError(res, error, 'controller.share.delete_user', 'deleteUser failed')
    }
  }
}
