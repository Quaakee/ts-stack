import { Transaction, type CreateActionArgs, type UnlockingScript, type WalletInterface } from '@bsv/sdk'
...
function inputOutpoint(transaction: Transaction, index: number): string {
  ...
  if (
    ...
    !Number.isSafeInteger(input.sourceOutputIndex) || input.sourceOutputIndex < 0 ||
    input.sourceOutputIndex > 0xffffffff
  ) throw new Error('Wallet transaction input is invalid')
  if (
    input.sourceTXID !== undefined && input.sourceTransaction !== undefined &&
...
function sameTemplate(partial: Transaction, signed: Transaction): boolean {
  if (
    partial.version !== signed.version || partial.lockTime !== signed.lockTime ||
    partial.inputs.length !== signed.inputs.length || partial.outputs.length !== signed.outputs.length
  ) return false
  for (let index = 0; index < partial.inputs.length; index++) {
    if (
      inputOutpoint(partial, index) !== inputOutpoint(signed, index) ||
      (partial.inputs[index].sequence ?? 0xffffffff) !== (signed.inputs[index].sequence ?? 0xffffffff)
    ) return false
  }
...
async function abort(wallet: WalletInterface, reference: string): Promise<void> {
  try { await wallet.abortAction({ reference }) } catch { /* preserve primary failure */ }
}
...
export async function completeUhrpAction(
  ...
  if (
    ...
    ((args.inputs?.length ?? 0) === 1) !== (signer !== undefined) ||
    (signer !== undefined && args.inputs?.[0]?.outpoint.toLowerCase() !== signer.outpoint.toLowerCase())
  ...
  try {
    ...
    let expectedScript: string | undefined
    let customIndex: number | undefined
    if (signer !== undefined) {
      const indexes = partial.inputs.flatMap((_, index) => inputOutpoint(partial, index) === signer.outpoint.toLowerCase() ? [index] : [])
      if (indexes.length !== 1) throw new Error('Wallet substituted the requested UHRP input')
      customIndex = indexes[0]
      if (
        (partial.inputs[customIndex].sequence ?? 0xffffffff) !==
        (args.inputs?.[0]?.sequenceNumber ?? 0xffffffff)
      ) throw new Error('Wallet substituted the requested UHRP input sequence')
      expectedScript = (await signer.sign(partial, customIndex)).toHex().toLowerCase()
      spends[customIndex] = { unlockingScript: expectedScript }
    }
    ...
    if (
      ...
      signed.inputs[customIndex].unlockingScript?.toHex().toLowerCase() !== expectedScript
    ) throw new Error('Wallet substituted the authorized UHRP unlocking script')
    if (
      signedResult.txid !== undefined &&
      (typeof signedResult.txid !== 'string' || signedResult.txid.toLowerCase() !== signed.id('hex').toLowerCase())
