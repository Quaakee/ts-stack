...
function canonicalScript(lockingPublicKey: PublicKey, fields: number[][]): LockingScript {
  ...
  for (let remaining = fields.length; remaining > 1; remaining -= 2) chunks.push({ op: 0x6d })
  if (fields.length % 2 === 1) chunks.push({ op: 0x75 })
  return new LockingScript(chunks)
}

function readPositiveCompactSize(field: number[], name: string): number {
  if (field.length < 1 || field.length > 9) throw new Error(`UHRP advertisement ${name} is invalid`)
  const first = field[0]
  let value: bigint
  let length: number
  if (first < 0xfd) {
    value = BigInt(first); length = 1
  } else if (first === 0xfd) {
    value = BigInt(field[1] ?? 0) | (BigInt(field[2] ?? 0) << 8n); length = 3
  ...
  if (field.length !== length || value < 1n || value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`UHRP advertisement ${name} is invalid`)
  }
  return Number(value)
}

const equalBytes = (a: number[], b: number[]): boolean =>
...
export async function decodeAndVerifyUHRPAdvertisement(
  ...
  if (
    fields.some(field => field.length > MAX_URL_BYTES) ||
    fields.reduce((total, field) => total + field.length, 0) > MAX_URL_BYTES + 256
  ...
  if (!equalBytes(PublicKey.fromString(hostIdentityKey).toDER() as number[], fields[0])) {
  ...
  const hostedFileLocation = new TextDecoder('utf-8', { fatal: true }).decode(Uint8Array.from(fields[2]))
  if (fields[2].length < 1 || /\p{Cc}/u.test(hostedFileLocation)) throw new Error('UHRP advertisement URL is invalid')
  const location = new URL(hostedFileLocation)
  if (
    location.protocol !== 'https:' || location.hostname === '' || location.username !== '' ||
  ...
  if (
    fields[5].length < 8 || fields[5].length > 80 ||
    !equalBytes(Signature.fromDER(fields[5]).toDER() as number[], fields[5])
  ) throw new Error('UHRP advertisement signature is invalid')
  const anyone = new ProtoWallet('anyone')
  const { publicKey } = await anyone.getPublicKey({ protocolID: PROTOCOL, keyID: '1', counterparty: hostIdentityKey })
  ...
  const { valid } = await anyone.verifySignature({
    data: fields.slice(0, 5).flat(), signature: fields[5], protocolID: PROTOCOL,
    keyID: '1', counterparty: hostIdentityKey
  })
  if (valid !== true) throw new Error('UHRP advertisement signature is invalid')
  return {
    hostIdentityKey, hash: [...fields[1]], hostedFileLocation, expiryTime, fileSize,
    lockingPublicKey, signature: [...fields[5]]
  }
}
